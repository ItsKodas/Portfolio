...
function isSettings(value: unknown): value is SiteSettings {
    ...
    const { capabilities, repo, credential, branches, websockets, ...rest } = value as Record<string, unknown>
    ...
    if (websockets !== undefined) {
        if (typeof websockets !== 'object' || websockets === null || Array.isArray(websockets)) return false
        if (!Object.values(websockets).every(enabled => typeof enabled === 'boolean')) return false
    }
