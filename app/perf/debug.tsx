...
interface Stats { frames: number, scrolls: number, worst: number, scene: string }
...
export default function PerfDebug() {
    ...
    useEffect(() => {
        ...
        const every = window.setInterval(() => {
            setStats({ frames, scrolls, worst: Math.round(worst), scene: document.documentElement.dataset.scene ?? '' })
            frames = 0
            scrolls = 0
            worst = 0
        }, 1000)
