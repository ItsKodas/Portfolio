...
describe('call', () => {
    ...
    it('passes a refusal through unchanged', async () => {
        ...
        assert.deepEqual(await client.call({ verb: 'fetch', dir: '/var/www/acme' }), { ok: false, code: 'failed', message: 'fatal: repository not found' })
    })
    ...
    it('throws FetcherUnavailableError when the socket closes with no reply', async () => {
        ...
        await assert.rejects(client.call({ verb: 'fetch', dir: '/var/www/acme' }), (error: unknown) => {
    ...
    it('throws FetcherUnavailableError on timeout, and closes the socket', async () => {
        ...
        await assert.rejects(client.call({ verb: 'fetch', dir: '/var/www/acme' }), /the fetcher did not answer within 0\.03 seconds/)
    ...
    it('throws FetcherUnavailableError when the socket fails', async () => {
        ...
        await assert.rejects(client.call({ verb: 'fetch', dir: '/var/www/acme' }), /the fetcher connection failed: connect ENOENT \/run\/hostd\/fetch\.sock/)
    })
    ...
    it('throws FetcherUnavailableError on a reply that is not JSON', async () => {
        ...
        await assert.rejects(client.call({ verb: 'fetch', dir: '/var/www/acme' }), /the fetcher sent an unreadable reply/)
    })
})
